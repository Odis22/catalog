...
@app.route('/store/<int:storet_id>/Catalog/<int:Catalog_id>/JSON')
...
def newCatalogItem(store_id):
    ...
    if login_session['user_id'] != store.user_id:
        return "<script>function myFunction() {alert('You are not authorized to add Catalog items to this store. Please create your own store in order to add items.');}</script><body onload='myFunction()''>"
        if request.method == 'POST':
...
def editCatalogItem(store_id, catalog_id):
    ...
    store = session.query(Store).filter_by(id=store_id).one()
    if login_session['user_id'] != store.user_id:
        return "<script>function myFunction() {alert('You are not authorized to edit Catalog items to this store. Please create your own store in order to edit items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
        if request.form['name']:
...
def deleteCatalogItem(store_id, Catalog_id):
    ...
    if login_session['user_id'] != store.user_id:
        return "<script>function myFunction() {alert('You are not authorized to delete Catalog items to this store. Please create your own store in order to delete items.');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
